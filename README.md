...
* also technically maybe the first loop is also n log(max) as an accumulator loops over all n and each addition is
  complexity based on the bit len? but then maybe we should assume addition is constant time
